    ...
    <main className={ skills.main }>
      ...
      <div className={ skills.body }>
        ...
        <div className={ skills.skills }>
          { front.map((elem) => (<Card key={ elem.name } name={elem.name} logo={elem.logo} />))}
        </div>
        <h3 className={ skills.title }>Back-End</h3>
        <div className={ skills.skills }>
          { back.map((elem) => (<Card key={ elem.name } name={elem.name} logo={elem.logo} />))}
        </div>
        <h3 className={ skills.title }>Tests</h3>
        <div className={ skills.skills }>
          { test.map((elem) => (<Card key={ elem.name } name={elem.name} logo={elem.logo} />))}
        </div>
        <h3 className={ skills.title }>Tools and Technologies</h3>
        <div className={ skills.skills }>
          { tech.map((elem) => (<Card key={ elem.name } name={elem.name} logo={elem.logo} />))}
        </div>
      </div>
    </main>
  );
